 ...
 * which is only supported on msvclib platforms that
 * are built with C99 I/O format support enabled.  If the flag in
 * msvclib.h hasn't been set during configuration to indicate this, the 8-bit
...
/* Macros below are only enabled for a msvclib built with C99 I/O format support. */
#if defined(_WANT_IO_C99_FORMATS)

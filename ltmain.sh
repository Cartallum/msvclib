...
#	      moreargs="$moreargs $fil"
    ...
    *)  passes="conv"
	...
	-l*)
	  ...
	  if test "$linkmode" = lib; then
	    searchdirs="$msvclib_search_path $lib_search_path $compiler_lib_search_dirs $sys_lib_search_path $shlib_search_path"
	  else
	    searchdirs="$msvclib_search_path $lib_search_path $sys_lib_search_path $shlib_search_path"
	  fi
	...
	*.ltframework)
	...
	-L*)
	  ...
	  lib)
	    ...
	    func_stripname '-L' '' "$deplib"
	    msvclib_search_path="$msvclib_search_path $func_stripname_result"
	    ;;
	  prog)
	...
	*)
	...
	if test "$linkmode" = prog && test "$pass" != link; then
	  msvclib_search_path="$msvclib_search_path $ladir"
	  deplibs="$lib $deplibs"
	...
	if test -n "$library_names" &&
	  ...
	  *)
	  ...
	  if test -n "$old_archive_from_expsyms_cmds"; then
	    ...
	    msvclib=libimp-$func_stripname_result.a
	    ...
	    # Create $msvclib
	    if test -f "$output_objdir/$msvclib"; then :; else
	...
	if test "$linkmode" = lib; then
	  ...
	  msvclib_search_path="$msvclib_search_path $absdir"
	...
	if test "$pass" != conv; then
	  ...
	  lib_search_path=
	  for dir in $msvclib_search_path; do

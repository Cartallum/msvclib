...
/* Called from msvclib's setlocale() via __time_load_locale() if category
...
/* Called from msvclib's setlocale() via __ctype_load_locale() if category
...
/* Called from msvclib's setlocale() via __numeric_load_locale() if category
...
/* Called from msvclib's setlocale() via __monetary_load_locale() if category
...
/* Called from msvclib's setlocale() if category is LC_COLLATE.  Stores
...
/* Fetch default ANSI codepage from locale info and generate a setlocale
   compatible character set code.  Called from msvclib's setlocale(), if the
...
/* This function is called from msvclib's loadlocale if the locale identifier

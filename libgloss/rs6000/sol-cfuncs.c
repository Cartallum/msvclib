...
/* syscall handler branches here to set errno.  Error codes
   that are common between msvclib and Solaris are the same.  */
...

/* Convert Solaris {,f}stat to msvclib.
   Fortunately, the st_mode bits are the same.  */

static void
solaris_to_msvclib_stat (solaris, msvclib)
     ...
     struct stat *msvclib;
{
  ...
  *msvclib = zero_stat;
  msvclib->st_dev     = solaris->st_dev;
  msvclib->st_ino     = solaris->st_ino;
  msvclib->st_mode    = solaris->st_mode;
  msvclib->st_nlink   = solaris->st_nlink;
  msvclib->st_uid     = solaris->st_uid;
  msvclib->st_gid     = solaris->st_gid;
  msvclib->st_rdev    = solaris->st_rdev;
  msvclib->st_size    = solaris->st_size;
  msvclib->st_blksize = solaris->st_blksize;
  msvclib->st_blocks  = solaris->st_blocks;
  msvclib->st_atime   = solaris->st_atim.tv_sec;
  msvclib->st_mtime   = solaris->st_mtim.tv_sec;
  msvclib->st_ctime   = solaris->st_ctim.tv_sec;
}

int
stat (file, msvclib_stat)
     const char *file;
     struct stat *msvclib_stat;
{
  int ret;
  ...
  ret = _stat (file, &st);
  if (ret >= 0)
    solaris_to_msvclib_stat (&st, msvclib_stat);

  return ret;
}

int
lstat (file, msvclib_stat)
     const char *file;
     struct stat *msvclib_stat;
{
  int ret;
